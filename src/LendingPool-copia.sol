...
contract LendingPool{
    ...
    function createPool(uint256 _poolId, uint256 _initialSupply, uint256 initialDebt) public onlyOwner {
    ...
    function repay(uint256 idBorrow, uint256 _amount)payable public {
        ...
        if (idBorrow == 2){
            ...
            (bool success) = wlink.transferFrom(msg.sender, address(this), amount);
            if(!success){
                revert TransferFailed();
            }
        }
        if (idBorrow == 3){
            ...
            (bool success) = wusdt.transferFrom(msg.sender, address(this), amount);
            if(!success){
                revert TransferFailed();
            }
        }
